    ...
    <div id="vue-app" class="lobby-container">
        ...
        <ul>
            <li v-on:click="searchInput('name')" class="name">
                ...
                <input v-if="inputType == 'name'" v-model="searchKey" type="search" class="search"
...
export default defineComponent({
  ...
  mounted () {
    const url = 'http://localhost/Formation/Projet/TP/Exemple/vuejs-php-master/libraries/controllers/getDataWines.php'
    axios
    //      .get("libraries/controllers/getDataWines.php")
    // , { params: { answer: 42 } }
    // Vue -> http / localhost -> PHP -> Select
    // Vue -> http / localhost -> mail ->  imap
    // JS -> Client VueJS (Firefox) || localhost (XAMP) <- PHP ici
